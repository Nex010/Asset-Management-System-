...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    error_log("====== Starting new form submission ======");
    error_log("POST data received: " . print_r($_POST, true));
    // Get form data with validation
    $asset_name = mysqli_real_escape_string($conn, $_POST['AssetName']);
    $serial_number = mysqli_real_escape_string($conn, $_POST['SerialNumber']);
    $category_id = !empty($_POST['CatergoryID']) ? mysqli_real_escape_string($conn, $_POST['CatergoryID']) : null;
    $subcategory_id = !empty($_POST['SubCatergoryID']) ? mysqli_real_escape_string($conn, $_POST['SubCatergoryID']) : null;
    $brand_id = !empty($_POST['BrandID']) ? mysqli_real_escape_string($conn, $_POST['BrandID']) : null;
    $user_id = !empty($_POST['UserID']) ? mysqli_real_escape_string($conn, $_POST['UserID']) : null;
    $status = mysqli_real_escape_string($conn, $_POST['st']); // Required ENUM field
    $purchase_date = !empty($_POST['PuchaseDate']) ? mysqli_real_escape_string($conn, $_POST['PuchaseDate']) : null; // Note spelling
    $purchase_cost = !empty($_POST['PurchaseCost']) ? mysqli_real_escape_string($conn, $_POST['PurchaseCost']) : 0.00; // Required decimal
    $asset_tag = mysqli_real_escape_string($conn, $_POST['AssetTag']); // Required varchar
    $order_id = !empty($_POST['OrderID']) ? mysqli_real_escape_string($conn, $_POST['OrderID']) : null;
    $ap_id = !empty($_POST['APID']) ? mysqli_real_escape_string($conn, $_POST['APID']) : null;
    $location = !empty($_POST['LocationID']) ? mysqli_real_escape_string($conn, $_POST['LocationID']) : null;
    $supplier_id = !empty($_POST['SupplierID']) ? mysqli_real_escape_string($conn, $_POST['SupplierID']) : null;
    ...
    // Bind parameters
    $bind_result = $stmt->bind_param("ssiiiiissdssiiii",
        $asset_name,
        $serial_number,
        $category_id,
        $subcategory_id,
    ...
    try {
        ...
        error_log("Error in asset insertion: " . $e->getMessage());
        echo "<script>
            Swal.fire({
                title: 'Error!',
                text: 'Failed to add asset: " . $e->getMessage() . "',
                icon: 'error',
                showCancelButton: false,
...
function getDropdownData($conn, $table, $id_field, $name_field) {
    $query = "SELECT $id_field, $name_field FROM $table WHERE 1";
...
$categories = getDropdownData($conn, 'categories', 'CatergoryID', 'CategoryName');
$subcategories = getDropdownData($conn, 'subcatergory', 'SubCatergoryID', 'CatergoryName');
$brands = getDropdownData($conn, 'brands', 'BrandID', 'BrandName');
$users = getDropdownData($conn, 'users', 'UserID', 'Username');
$suppliers = getDropdownData($conn, 'suppliers', 'SupplierID', 'SupplierName');
$aps = getDropdownData($conn, 'areaprogram', 'ApID', 'ApName');
$locations = getDropdownData($conn, 'location', 'LocationID', 'LocationName');
...
<img alt="" src="assets/img/User1.png">
</span>
...
<ul>
<li><a href="productlist.php">Assets List</a></li>
<li><a href="addproduct.php" class="active">Add Asset</a></li>
...
<li class="submenu">
<a href="javascript:void(0);"><img src="assets/img/icons/quotation1.svg" alt="img"><span> Oders</span> <span class="menu-arrow"></span></a>
<ul>
<li><a href="quotationList.php">Order List</a></li>
        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-sm-6 col-12">
                <div class="form-group">
                    ...
                    <select class="select" name="CatergoryID" required>
                        <option value="">Choose Category</option>
                        <?php foreach($categories as $category): ?>
                            <option value="<?php echo $category['CatergoryID']; ?>">
            ...
            <div class="col-lg-3 col-sm-6 col-12">
                <div class="form-group">
                    ...
                    <select class="select" name="SubCatergoryID">
                        <option value="">Choose Sub Category</option>
                        <?php foreach($subcategories as $subcategory): ?>
                            <option value="<?php echo $subcategory['SubCatergoryID']; ?>">
                                <?php echo htmlspecialchars($subcategory['CatergoryName']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div class="col-lg-3 col-sm-6 col-12">
                <div class="form-group">
                    ...
                    <div class="input-groupicon">
                        <input type="text" name="PuchaseDate" placeholder="DD-MM-YYYY" class="datetimepicker">
                        <div class="addonset">
            ...
            <div class="col-lg-3 col-sm-6 col-12">
                <div class="form-group">
                    ...
                    <input type="text" name="AssetTag" class="form-control" required>
                </div>
            </div>
            <div class="col-lg-12">
                <div class="form-group">
                    <label>Description</label>
                    <textarea name="Description" class="form-control" rows="4" placeholder="Enter asset description"></textarea>
                </div>
            </div>
            <div class="col-lg-3 col-sm-6 col-12">
                <div class="form-group">
...
<div class="col-lg-3 col-sm-6 col-12">
                <div class="form-group">
                    ...
                    <input type="number" name="PurchaseCost" class="form-control" step="0.01" required>
                </div>
            </div>
            <div class="col-lg-3 col-sm-6 col-12">
                <div class="form-group">
            ...
            <div class="col-lg-3 col-sm-6 col-12">
                <div class="form-group">
                    <label>Status</label>
                    <select class="select" name="st" required>
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelector('form').addEventListener('submit', function (e) {
        ...
        const requiredFields = ['AssetName', 'CatergoryID', 'st', 'AssetTag', 'PurchaseCost'];
        let missingFields = [];
        
        requiredFields.forEach(field => {
            const element = this.querySelector(`[name="${field}"]`);
            if (!element || !element.value.trim()) {
                missingFields.push(field); // Push field names if empty
            }
        });
